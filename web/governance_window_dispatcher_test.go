...
func TestGovernanceWindowDispatcherPreservesExplicitAuditTargetAcrossNonTradingExecutionDay(t *testing.T) {
	originalStore := governanceStore
	originalPaths := governancePaths
	originalAudit := dailyAudit
	originalDispatcher := governanceWindowDispatcher
	...
	now := time.Date(2026, 5, 1, 0, 28, 54, 0, time.Local)
	executedDates := make([]string, 0, 2)
	dailyAudit, err = systemgov.NewDailyAuditRunner(systemgov.DailyAuditConfig{
		Store: store,
		Paths: paths,
		...
		ResolveTargetDates: func(ctx context.Context, now time.Time) ([]string, error) {
			t.Fatalf("dispatcher should execute audit with explicit dates from the durable window")
			return nil, nil
		},
		Execute: func(ctx context.Context, date string, trigger string) (*systemgov.AuditResult, error) {
			executedDates = append(executedDates, date)
	...
	if len(executedDates) != 2 || executedDates[0] != "20260429" || executedDates[1] != "20260430" {
		t.Fatalf("executed dates = %+v, want [20260429 20260430]", executedDates)
	}
	window, err := store.GetWindowByKey(auditKey)
	if err != nil {
		t.Fatalf("get audit window: %v", err)
	...
	if window == nil || window.Status != collectorpkg.GovernanceWindowStatusPassed {
		t.Fatalf("audit window = %+v, want passed", window)
	}
	if !strings.Contains(window.ResultSummary, "target=20260429,20260430") {
		t.Fatalf("result summary = %q, want explicit target window", window.ResultSummary)
	}
	if strings.Contains(window.ResultSummary, "target=20260501") {
		t.Fatalf("result summary used execution day instead of target window: %q", window.ResultSummary)
	}
}
