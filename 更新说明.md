...
Docker 更新流程请统一参考 `DOCKER_DEPLOY.md`。当前正式流程是先构建最终镜像，再用 compose 部署：

```bash
./scripts/build_local_image.sh
docker compose up -d --force-recreate stock-web
...
```bash
./scripts/build_local_image.sh && docker compose up -d --force-recreate stock-web
```
