...
func parseConfig(args []string) (cliConfig, error) {
	...
	fs.StringVar(&cfg.Repair, "repair", "", "repair operation to run: stale-lock, startup-recovery-deferred, terminal-windows, covered-close-sync-windows, covered-audit-windows, covered-backlog, degraded-provider-backlog")
	...
	if cfg.Repair != "" && cfg.Repair != "stale-lock" && cfg.Repair != "startup-recovery-deferred" && cfg.Repair != "terminal-windows" && cfg.Repair != "covered-close-sync-windows" && cfg.Repair != "covered-audit-windows" && cfg.Repair != "covered-backlog" && cfg.Repair != "degraded-provider-backlog" {
...
func governanceRepairOperations(cfg cliConfig) []collectorpkg.GovernanceRepairOperation {
	...
	case "startup-recovery-deferred":
		return []collectorpkg.GovernanceRepairOperation{
			collectorpkg.StartupRecoveryDeferredBacklogRepair{},
		}
	case "terminal-windows":
		return []collectorpkg.GovernanceRepairOperation{
			collectorpkg.TerminalGovernanceWindowRepair{},
		}
	case "covered-close-sync-windows":
		return []collectorpkg.GovernanceRepairOperation{
			collectorpkg.CoveredCloseSyncWindowRepair{},
		}
	case "covered-audit-windows":
		return []collectorpkg.GovernanceRepairOperation{
			collectorpkg.CoveredAuditWindowRepair{},
		}
	case "covered-backlog":
		return []collectorpkg.GovernanceRepairOperation{
