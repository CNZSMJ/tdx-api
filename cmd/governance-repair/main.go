...
func parseConfig(args []string) (cliConfig, error) {
	...
	fs.StringVar(&cfg.Repair, "repair", "", "repair operation to run: stale-lock, startup-recovery-deferred, terminal-windows, covered-close-sync-windows, degraded-provider-backlog")
	...
	if cfg.Repair != "" && cfg.Repair != "stale-lock" && cfg.Repair != "startup-recovery-deferred" && cfg.Repair != "terminal-windows" && cfg.Repair != "covered-close-sync-windows" && cfg.Repair != "degraded-provider-backlog" {
