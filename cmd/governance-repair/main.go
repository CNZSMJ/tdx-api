...
func parseConfig(args []string) (cliConfig, error) {
	...
	fs.StringVar(&cfg.Repair, "repair", "", "repair operation to run: stale-lock, startup-recovery-deferred")
	...
	if cfg.Repair != "" && cfg.Repair != "stale-lock" && cfg.Repair != "startup-recovery-deferred" {
